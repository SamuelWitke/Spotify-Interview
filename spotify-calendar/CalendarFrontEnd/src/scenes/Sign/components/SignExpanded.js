...
class SignExpanded extends Component {
	...
	render () {
		...
		return (
			...
			{({flexVal}) =>
			<div className={ type === 'signIn' ? 'signInExpanded' : 'signUpExpanded'} style={{
				flexGrow: `${flexVal}`
			}}>
				<Motion style={{ 
					opacity: spring(flexState ? 1 : 0,{stiffness: 300, damping: 17}),
					y: spring(flexState ? 0 : 50, {stiffness: 100, damping: 17})
				}} >
					{({opacity, y}) =>
					<form 
						onSubmit={ type === "signIn" ? this.onSubmitSignIn : this.onSubmitSignUp }
