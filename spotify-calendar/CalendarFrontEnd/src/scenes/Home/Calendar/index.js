...
class Calander extends React.Component {
	...
	onSubmit(){
		const {img, edit, eventEndTime, eventStartTime, eventDescription, selected } = this.state;
		if(edit){
			const body = {
				id: selected.key,
				date: selected.date,
			...
			const body = {
				...
				eventEndTime,
				eventDescription,
				img: img == null ? `https://source.unsplash.com/collection/${190727}/${480}${480}/?sig=${Math.floor(Math.random())+selected.number}` : img,
			}
	...
	componentDidUpdate(nextProps,nextState) {
		if(this.state.open !== nextState.open) {this.apiEvents();}
	}
		...
		return (
			<div className={classes.root}>
					...
					<Grid item xs={12}>
						<Paper className={classes.paper}>
							...
							<Card>
								...
								<CardActions>
									<FlatButton label={<span> <MdArrowBack size={20}/> Previous </span>} onClick={this.props.previousMonth}/>
									<FlatButton label={<span> Next <MdArrowForward size={20}/> </span>} onClick={this.props.nextMonth} />
								</CardActions>
				...
				<Grid container spacing={24}>
					<Grid item xs={12}>
						<Paper className={classes.paper}>
							<EventList tilesData={apiEvents} onEdit={this.onEditOpen} onDelete={this.onDelete}/>
						</Paper>
					</Grid>
				</Grid>
			</div>
